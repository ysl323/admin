...
$testData = (Get-Content "test-data.json" -Raw).Replace("`r`n", "").Replace("`n", "")
...
$testCategory = $categoriesResponse.categories | Where-Object { $_.name -eq "测试导入分类" }
...
$invalidData = '{"category":"Invalid","lessons":[{"lesson":1,"words":[{"en":"test"}]}]}'

try {
    $invalidResponse = Invoke-RestMethod -Uri "$baseUrl/admin/import-json-direct" -Method POST -Body $invalidData -ContentType "application/json" -WebSession $session -ErrorAction Stop

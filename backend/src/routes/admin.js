...
router.delete('/users/:id', async (req, res) => {
  try {
    ...
    const result = await UserService.deleteUser(userId, operatorId);
    ...
    if (error.message === '不能删除自己的账号' || error.message === '不能删除超级管理员') {
      return res.status(403).json({
        success: false,
        message: error.message
      });
 ...
 * 导出所有课程数据（与导入格式一致）
 * 格式：[{categoryName, words: [{lesson, question, english, phonetic, chinese}]}]
 */

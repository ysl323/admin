...
Write-Host "Safe Deployment of Export Feature" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
try {
    ...
    # Manually add export functions to AdminService.js
    Write-Host "`nAdding export functions to AdminService.js..." -ForegroundColor Gray
...
'@
    ...
    # Escape the export functions for shell
    $escapedFunctions = $exportFunctions -replace "'", "'\''" -replace "`", "\\`" -replace '"', '\"'
...
echo '$escapedFunctions' >> AdminService.js.tmp
...
'@
    ...
    $escapedRoutes = $exportRoutes -replace "'", "'\''" -replace "`", "\\`" -replace '"', '\"'

    $updateRoutesCmd = @"
cd /root/english-learning/backend/src/routes
head -n -1 admin.js > admin.js.tmp
echo '$escapedRoutes' >> admin.js.tmp
echo '' >> admin.js.tmp

...
Write-Host "Clean Deployment - Replace Entire Files" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
try {
    ...
    # Create temp files with export functions
    Write-Host "`n[3/4] Preparing export functions..." -ForegroundColor Gray

    # Export functions for AdminService.js (append before export default)
    $adminServiceExport = @'
...
'@
    ...
    # Export routes for admin.js (append before export default)
    $adminRoutesExport = @'
...
'@
    ...
    # Upload export functions to temp files
    Write-Host "Uploading export functions to temp files..." -ForegroundColor Gray

    Invoke-SSHCommand -SessionId $session.SessionId -Command "cat > /tmp/adminServiceExport.js << 'EOF'
$adminServiceExport
EOF"

    Invoke-SSHCommand -SessionId $session.SessionId -Command "cat > /tmp/adminRoutesExport.js << 'EOF'
$adminRoutesExport
EOF"

    # Insert export functions before the export default line
    Write-Host "Inserting export functions..." -ForegroundColor Gray

    Invoke-SSHCommand -SessionId $session.SessionId -Command "cd /root/english-learning/backend/src/services && sed -i '/^export default/i cat /tmp/adminServiceExport.js' AdminService.js"
    Invoke-SSHCommand -SessionId $session.SessionId -Command "cd /root/english-learning/backend/src/routes && sed -i '/^export default/i cat /tmp/adminRoutesExport.js' admin.js"

    Write-Host "Export functions added" -ForegroundColor Green
